...
!===============================================================================
  ...
  subroutine aerocom(Cam, Nnatk, pint, coszrs)
    ...
    real(r8) :: dod550dry(pcols)
    real(r8) :: abs550dry(pcols)
    ...
    real(r8) :: dload_mi(pcols)
    real(r8) :: dload_ss(pcols)
    real(r8) :: dload_s4(pcols)
    real(r8) :: dload_oc(pcols)
    real(r8) :: dload_bc(pcols)
    real(r8) :: dload_s4_a(pcols)
    real(r8) :: dload_s4_1(pcols)
    real(r8) :: dload_s4_5(pcols)

    real(r8) :: dload_bc_0(pcols)
    real(r8) :: dload_bc_ac(pcols)
    real(r8) :: dload_oc_ac(pcols)
    real(r8) :: dload_bc_2(pcols)
    real(r8) :: dload_bc_4(pcols)
    real(r8) :: dload_bc_12(pcols)
    real(r8) :: dload_bc_14(pcols)
    real(r8) :: dload_oc_4(pcols)
    real(r8) :: dload_oc_14(pcols)

    real(r8) :: cmin(pcols,pver)
    real(r8) :: cseas(pcols,pver)

    real(r8) :: nnat_0(pcols,pver)
    real(r8) :: nnat_1(pcols,pver)
    real(r8) :: nnat_2(pcols,pver)
    real(r8) :: nnat_3(pcols,pver)
    real(r8) :: nnat_4(pcols,pver)
    real(r8) :: nnat_5(pcols,pver)
    real(r8) :: nnat_6(pcols,pver)
    real(r8) :: nnat_7(pcols,pver)
    real(r8) :: nnat_8(pcols,pver)
    real(r8) :: nnat_9(pcols,pver)
    real(r8) :: nnat_10(pcols,pver)
    real(r8) :: nnat_12(pcols,pver)
    real(r8) :: nnat_14(pcols,pver)

    real(r8) :: ck(pcols,pver,0:nmodes)

    real(r8) :: c_mi(pcols,pver)
    real(r8) :: c_mi05(pcols,pver)
    real(r8) :: c_mi125(pcols,pver)
    real(r8) :: c_ss(pcols,pver)
    real(r8) :: c_ss05(pcols,pver)
    real(r8) :: c_ss125(pcols,pver)
    real(r8) :: c_bc(pcols,pver)
    real(r8) :: c_bc05(pcols,pver)
    real(r8) :: c_bc125(pcols,pver)
    real(r8) :: c_oc(pcols,pver)
    real(r8) :: c_oc05(pcols,pver)
    real(r8) :: c_oc125(pcols,pver)
    real(r8) :: c_sa(pcols,pver)
    real(r8) :: c_sa05(pcols,pver)
    real(r8) :: c_sa125(pcols,pver)
    real(r8) :: c_sc(pcols,pver)
    real(r8) :: c_sc05(pcols,pver)
    real(r8) :: c_sc125(pcols,pver)
    real(r8) :: c_s4(pcols,pver)
    real(r8) :: c_s405(pcols,pver)
    real(r8) :: c_s4125(pcols,pver)
    real(r8) :: c_s4_a(pcols,pver)
    real(r8) :: c_s4_1(pcols,pver)
    real(r8) :: c_s4_5(pcols,pver)

    real(r8) :: c_bc_0(pcols,pver)
    real(r8) :: c_bc_ac(pcols,pver)
    real(r8) :: c_oc_ac(pcols,pver)
    real(r8) :: c_bc_2(pcols,pver)
    real(r8) :: c_bc_4(pcols,pver)
    real(r8) :: c_bc_12(pcols,pver)
    real(r8) :: c_bc_14(pcols,pver)
    real(r8) :: c_oc_4(pcols,pver)
    real(r8) :: c_oc_14(pcols,pver)
    ...
    real(r8) :: bebglt1(pcols,pver,0:nbmodes)
    real(r8) :: bebggt1(pcols,pver,0:nbmodes)
    real(r8) :: bebclt1(pcols,pver,0:nbmodes)
    real(r8) :: bebcgt1(pcols,pver,0:nbmodes)
    real(r8) :: beoclt1(pcols,pver,0:nbmodes)
    real(r8) :: beocgt1(pcols,pver,0:nbmodes)
    real(r8) :: bes4lt1(pcols,pver,0:nbmodes)
    real(r8) :: bes4gt1(pcols,pver,0:nbmodes)
    real(r8) :: backsc550(pcols,pver,0:nbmodes)
    real(r8) :: backsc550x(pcols,pver,nbmp1:nmodes)
    real(r8) :: backsc550tot(pcols,pver)

    ! Additional AeroCom Phase III output:
    real(r8) asydry_aer(pcols,pver)    ! dry asymtot in the visible band
    !
    real(r8) :: ec550_so4(pcols,pver)
    real(r8) :: ec550_bc(pcols,pver)
    real(r8) :: ec550_pom(pcols,pver)
    real(r8) :: ec550_ss(pcols,pver)
    real(r8) :: ec550_du(pcols,pver)
    real(r8) :: ec550_aer(pcols,pver)
    real(r8) :: abs550_aer(pcols,pver)
    ...
    real(r8) :: bebg440tot(pcols,pver)
    real(r8) :: bebg500tot(pcols,pver)
    real(r8) :: bebg550tot(pcols,pver), babg550tot(pcols,pver)
    real(r8) :: bebg670tot(pcols,pver)
    real(r8) :: bebg870tot(pcols,pver)
    real(r8) :: bebc440tot(pcols,pver)
    real(r8) :: bebc500tot(pcols,pver)
    real(r8) :: bebc550tot(pcols,pver), babc550tot(pcols,pver)
    real(r8) :: bebc670tot(pcols,pver)
    real(r8) :: bebc870tot(pcols,pver)
    real(r8) :: beoc440tot(pcols,pver)
    real(r8) :: beoc500tot(pcols,pver)
    real(r8) :: beoc550tot(pcols,pver), baoc550tot(pcols,pver)
    real(r8) :: beoc670tot(pcols,pver)
    real(r8) :: beoc870tot(pcols,pver)
    real(r8) :: besu440tot(pcols,pver)
    real(r8) :: besu500tot(pcols,pver)
    real(r8) :: besu550tot(pcols,pver), basu550tot(pcols,pver)
    real(r8) :: besu670tot(pcols,pver)
    real(r8) :: besu870tot(pcols,pver)
    ...
    real(r8) :: dod440_ss(pcols), dod440_dust(pcols), dod440_so4(pcols), dod440_bc(pcols), dod440_pom(pcols)
    real(r8) :: dod500_ss(pcols), dod500_dust(pcols), dod500_so4(pcols), dod500_bc(pcols), dod500_pom(pcols)
    real(r8) :: dod550_ss(pcols), dod550_dust(pcols), dod550_so4(pcols), dod550_bc(pcols), dod550_pom(pcols)
    real(r8) :: dod670_ss(pcols), dod670_dust(pcols), dod670_so4(pcols), dod670_bc(pcols), dod670_pom(pcols)
    real(r8) :: dod870_ss(pcols), dod870_dust(pcols), dod870_so4(pcols), dod870_bc(pcols), dod870_pom(pcols)

    real(r8) :: dod550lt1_ss(pcols)  , dod550gt1_ss(pcols)
    real(r8) :: dod550lt1_dust(pcols), dod550gt1_dust(pcols)
    real(r8) :: dod550lt1_so4(pcols) , dod550gt1_so4(pcols)
    real(r8) :: dod550lt1_bc(pcols)  , dod550gt1_bc(pcols)
    ...
    real(r8) :: akcxs(pcols)
    real(r8) :: rh0(pcols,pver)
    real(r8) :: rhoda(pcols,pver)
    ...
    real(r8) :: betotvis(pcols,pver)
    real(r8) :: batotvis(pcols,pver)
    ...
    integer  :: ifombg1(pcols,pver)
    integer  :: ifombg2(pcols,pver)
    ...
    integer  :: irh2(pcols,pver)
    integer  :: irh1null(pcols,pver)
    integer  :: irh2null(pcols,pver)
    ...
    call opticsAtConstRh(lchnk, ncol, pint, rhoda,    &
         Nnatk, xrh, irh1,                            &
         xct, ict1, xfaq, ifaq1, xfbcbg, ifbcbg1,     &
         xfbcbgn, ifbcbgn1, xfac, ifac1, xfbc, ifbc1, &
         xfombg, ifombg1, vnbcarr, vaitbcarr, v_soana, irf=0)
    ...
    do icol=1,ncol
       do k=1,pver
          ...
          dod4403d(icol,k) = dod4403d_ss(icol,k)+dod4403d_dust(icol,k) &
               +dod4403d_so4(icol,k)+dod4403d_bc(icol,k) +dod4403d_pom(icol,k)
          dod5003d(icol,k) = dod5003d_ss(icol,k)+dod5003d_dust(icol,k) &
               +dod5003d_so4(icol,k)+dod5003d_bc(icol,k) +dod5003d_pom(icol,k)
          dod5503d(icol,k) = dod5503d_ss(icol,k)+dod5503d_dust(icol,k) &
               +dod5503d_so4(icol,k)+dod5503d_bc(icol,k) +dod5503d_pom(icol,k)
          dod6703d(icol,k) = dod6703d_ss(icol,k)+dod6703d_dust(icol,k) &
               +dod6703d_so4(icol,k)+dod6703d_bc(icol,k) +dod6703d_pom(icol,k)
          dod8703d(icol,k) = dod8703d_ss(icol,k)+dod8703d_dust(icol,k) &
               +dod8703d_so4(icol,k)+dod8703d_bc(icol,k) +dod8703d_pom(icol,k)
          abs5503d(icol,k) = abs5503d_ss(icol,k)+abs5503d_dust(icol,k) &
               +abs5503d_so4(icol,k)+abs5503d_bc(icol,k) +abs5503d_pom(icol,k)
    ...
    ! BC(ax) mode
    call aerodry_prop%intdrypar0(ncol, Nnatk)

    ! SO4&SOA(Ait,n) mode
    call aerodry_prop%intdrypar1(ncol, Nnatk, xfombg, ifombg1, xct, ict1, xfac, ifac1)

    ! BC(Ait,n) and OC(Ait,n) modes
    call aerodry_prop%intdrypar2to3(ncol, Nnatk, xct, ict1, xfac, ifac1)

    ! BC&OC(Ait,n) mode   ------ fcm not valid here (=0). Use faitbc or fnbc instead
    call aerodry_prop%intdrypar4(ncol, Nnatk, xfbcbg, ifbcbg1, xfbcbgn, ifbcbgn1, &
         xct, ict1, xfac, ifac1, xfaq, ifaq1)

    ! SO4(Ait75) (5), mineral (6-7) and Sea-salt (8-10) modes:
    call aerodry_prop%intdrypar5to10(ncol, Nnatk, xct, ict1, xfac, ifac1, xfbc, ifbc1, xfaq, ifaq1)
    ...
    enddo
    call opticsAtConstRh(lchnk, ncol, pint, rhoda,    &
         Nnatk, xrhnull, irh1null,                    &
         xct, ict1, xfaq, ifaq1, xfbcbg, ifbcbg1,     &
         xfbcbgn, ifbcbgn1, xfac, ifac1, xfbc, ifbc1, &
         xfombg, ifombg1, vnbcarr, vaitbcarr, v_soana, irf=1)

  end subroutine aerocom

  !===============================================================================

  subroutine opticsAtConstRh (lchnk, ncol, pint, rhoda, &
       Nnatk, xrh, irh1,                                &
       xct, ict1, xfaq, ifaq1, xfbcbg, ifbcbg1,         &
       xfbcbgn, ifbcbgn1, xfac, ifac1, xfbc, ifbc1,     &
       xfombg, ifombg1, vnbc, vaitbc, v_soana, irf)
  ...
  subroutine intfrh (ncol, v3so4, v3insol, v3oc, v3ss, relh, frh)

    ! Originally written by Alf Kirkevaag in November 2011, based on interpol1to3 in optinterpol.F90
    ! Currently not being used - previously called by NorESM/physpkg
    ...
    real(r8) :: a, b, fso4, finsol, foc, fss
    real(r8) :: xrh(pcols,pver)
    integer  :: t_irh1, t_irh2
    real(r8) :: t_xrh, t_rh1, t_rh2
    integer, parameter :: e = 2.718281828

    ! Humidity growth factors which are consistent with the aerosol optics look-up tables:
    real(r8) :: fh_SO4(10)   = (/ 1.00_r8, 1.34_r8, 1.40_r8, 1.53_r8, 1.64_r8, 1.71_r8, 1.81_r8, 1.98_r8, 2.39_r8, 5.04_r8 /)
    real(r8) :: fh_insol(10) = (/ 1.00_r8, 1.01_r8, 1.01_r8, 1.02_r8, 1.02_r8, 1.02_r8, 1.02_r8, 1.02_r8, 1.02_r8, 1.02_r8 /)
    real(r8) :: fh_OC(10)    = (/ 1.00_r8, 1.02_r8, 1.05_r8, 1.14_r8, 1.19_r8, 1.22_r8, 1.27_r8, 1.36_r8, 1.59_r8, 3.18_r8 /)
    real(r8) :: fh_SS(10)    = (/ 1.00_r8, 1.01_r8, 1.02_r8, 1.56_r8, 1.87_r8, 1.97_r8, 2.12_r8, 2.35_r8, 2.88_r8, 6.08_r8 /)
    ! -----------------------------------------
